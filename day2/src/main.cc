...
#define F_COMPARE(arg) else if(std::strcmp(stat_buf, #arg) == 0){ \
  arg = current; \
}
...
struct Round {
  ...
  uint16_t read_round(const std::string_view buf, std::string_view::const_iterator& it){
    char stat_buf[6];
    while(it < buf.cend()){
      if(*it == ';')
        return 0;
...
struct Game {
  ...
  size_t read_game(const std::string_view buf, std::string_view::const_iterator& it){
    while(true){
      Round round;
